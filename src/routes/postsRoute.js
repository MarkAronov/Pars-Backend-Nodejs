...
router.patch('/posts/:id', auth, async function (req, res) {
  ...
  if (!updateKeys.every((key) => userParams.includes(key)))
  ...
  try {
    ...
    const currentParents = post.replyingParents
      .slice()
      .map((value) => value._id.toString());
    const updatedParents = req.body.parents_ids.slice();
    ...
    for (const updatedParentID of updatedParents) {
      ...
      if (updatedParent._id.euqals(post._id))
        return res.status(400).send('You cannot reply to yourself');
    }
    post.title = req.body.title;
    post.content = req.body.content;
    post.edited = true;
    for (const currParentId of post.replyingParents) {
    ...
    for (const newerParentId of updatedParents) {
      if (currentParents.indexOf(newerParentId) === -1) {
        ...
        newerParent.replyingChildren = newerParent.replyingChildren.concat(
          post._id
        );
        await newerParent.save();
      }
    }
    post.replyingParents = currentParents.map((parent) =>
